...
export class BootScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // UI chrome — 9-slice window panel loaded as an optional image.
    // When present it replaces the procedural fallback in DialogBox, Menu, and BaseScene.createPanel.
    // Source: assets/ui/window.png (48×48 px stub generated by scripts/generate-ui-stubs.js,
    //         or the real Ninja Adventure HUD panel upscaled 3× via scripts/upscale-assets.js).
    const optionalUiKeys = new Set(['ui_window'])

    // Ignore missing optional assets — audio files, loop-points JSON, character sprites, incident sprites, and UI chrome.
    ...
    this.load.on('loaderror', (file) => {
      ...
      const isOptionalUi       = file?.type === 'image' && optionalUiKeys.has(file.key)
    ...
    // Load UI chrome — 9-slice window panel (optional; falls back to procedural stub when absent).
