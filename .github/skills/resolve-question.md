...
Iterate design ideas for a Cloud Quest `[Design Question]` issue and find the best fit. Invoke with an issue number (e.g. "resolve question 35").

This skill reads the game's knowledge graph, wiki, content bible, and existing data files to propose concrete answers to open design questions — then stress-tests each proposal against the game's fun factor, architecture rules, and internal consistency before committing to a recommendation.
