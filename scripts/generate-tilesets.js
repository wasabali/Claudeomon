...
// ---------------------------------------------------------------------------
// Supplemental tileset definitions
// These are single-row strip tilesets used across many maps.
// ---------------------------------------------------------------------------

// stub_tiles — 5-tile base tileset referenced by every .tmj as firstgid=1.
// IDs: 1=grass, 2=building_a, 3=building_b, 4=building_c, 5=wall
const STUB_TILES = [
  tile( 74,124, 62,'grass'),  // 1 — grass floor (walkable)
  tile(100,100,110,'stone'),  // 2 — building A (stone wall exterior)
  tile( 80, 60, 40,'wood'),   // 3 — building B (wooden structure)
  tile( 50, 60, 80,'stone'),  // 4 — building C (dark stone structure)
...
const VOID_TILES = [
  ...
  tile( 30, 30, 63,'stone'),  //  3 — void_platform
  tile( 30, 30, 63,'flat'),   //  4 — void_platform_edge_l
  tile( 30, 30, 63,'flat'),   //  5 — void_platform_edge_r
  tile( 26, 26, 46,'cross'),  //  6 — void_star_dense
  tile( 42, 26, 62,'flat'),   //  7 — void_debris
  tile(  0,255,204,'flat'),   //  8 — void_glitch_h (cyan accent)
  tile(  0,255,204,'flat'),   //  9 — void_glitch_v (cyan accent)
...
const WASTELAND_TILES = [
  ...
  tile(154,154,138,'stone'),  //  3 — waste_concrete
  tile(122, 96, 40,'stone'),  //  4 — waste_rubble
  tile( 74, 90, 32,'grass'),  //  5 — waste_dead_grass
  tile(139, 58, 26,'wood'),   //  6 — waste_rusted_pipe
  tile( 90, 64, 48,'stone'),  //  7 — waste_server_rack
  tile(255,204,  0,'cross'),  //  8 — waste_caution_tape
  tile(255,204,  0,'flat'),   //  9 — waste_warning_sign
...
function generateStripPng(name, tiles, outDir) {
  ...
  for (let col = 0; col < cols; col++) {
    const t = tiles[col]
    paintTile(pixels, col, 0, width, t.r, t.g, t.b, t.style || 'flat')
  }

  const png = encodePng(width, height, pixels)
  fs.writeFileSync(path.join(outDir, `${name}.png`), png)
  console.log(`  wrote  ${name}.png (${cols}×1 tiles, ${width}×${height}px)`)
}

// ---------------------------------------------------------------------------
// Grid-tileset generator — multi-row tilesets (cols × rows)
// ---------------------------------------------------------------------------

function generateGridPng(name, tiles, cols, outDir) {
...
// Supplemental tilesets
console.log('Generating supplemental tilesets…')
generateStripPng('stub_tiles', STUB_TILES, MAPS_DIR)
generateStripPng('void_tiles', VOID_TILES, TILES_DIR)
generateStripPng('wasteland_tiles', WASTELAND_TILES, TILES_DIR)
// kenney_tech_office.png is generated by scripts/generate-tech-sprites.js
// which produces detailed pixel art tiles and individual sprite PNGs.
// Run: node scripts/generate-tech-sprites.js  (or: npm run gen:tech-sprites)
console.log('  calling generate-tech-sprites.js for kenney_tech_office…')
execFileSync(process.execPath, [path.join(__dirname, 'generate-tech-sprites.js')], { stdio: 'inherit' })
console.log()
